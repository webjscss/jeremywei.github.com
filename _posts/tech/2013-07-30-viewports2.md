...
这篇文章我们来聊聊关于移动浏览器的内容。如果你对移动开发完全是一个新手的话，我建议你先读一下[第一篇](http://weizhifeng.net/viewports.html)关于桌面浏览器的文章，为下面要讲的内容先熟悉下环境。

#移动浏览器的问题

当我们比较移动浏览器和桌面浏览器的时候，它们最显而易见的不同就是屏幕尺寸。为桌面浏览器所设计的网站在移动浏览器中显示的内容明显要少于桌面浏览器；不管是对其进行缩放直到文字小得无法阅读，还是在屏幕中只显示站点中的一小部分内容。

移动设备的屏幕比桌面屏幕要小得多；想想其最大有400px宽，有时候会小很多。（一些手机声称拥有更大的宽度，但是它在撒谎－或者也可以说给我们提供了没用的信息。）

平板设备中的像素中间层会在桌面环境和移动环境的缺口之间架起一段桥梁，比如像iPad或者传说中HP基于webOS研发的设备，但是这并没有改变根本问题。站点必须也能在移动设备上能工作，所以我们不得不让它们能在小尺寸的屏幕上正常显示。
...
解决这个问题的一个方法是为移动浏览器建立一个特定的站点。先讨论你有没有必要这么做这个基本问题，实际的情况是只有很少的网站拥有者真正知道要对移动设备做特殊的处理。
...
所以`<html>`元素在初始情况下用的是layout viewport的宽度，并且你的CSS也是在屏幕明显要比电话屏幕要宽的假设基础上进行解释的。这使得你站点布局的行为与其在桌面浏览器上的一样。
...
所以layout viewport的宽度和高度等于在最大限度缩小的模式下屏幕上所能显示的所有内容。当用户放大的时候这些尺寸保持原样。
...
![mobile_client_la](http://www.quirksmode.org/mobile/pix/viewport/mobile_client_la.jpg)
...
不幸的是这是浏览器不兼容的一部分；许多浏览器仍然不得不增加对visual viewport度量尺寸的支持。但是没有浏览器把这个度量尺寸存放任何其他的属性对中，所以我猜`window.innerWidth/Height`是标准，尽管它被支持的很糟。
...
![mobile_page](http://www.quirksmode.org/mobile/pix/viewport/mobile_page.jpg)
...
![mobile_pageXY](http://www.quirksmode.org/mobile/pix/viewport/mobile_pageXY.jpg)
...
（值得注意的例外是Android WebKit，它实际上会减小包含文字元素的大小，所以文字就能适配屏幕。这简直太有才了，我觉得所有其他浏览器应该借鉴这个行为。我过阵子将会完整的写一下这个议题。）
...
但这里有一个隐情。有时候正规的`screen.width`不那么明了，因为像素的数量太大了。比如，Nexus One的正规宽度是480px，但是Google的工程师们判断当使用`device-width`的时候，layout viewport的宽度为480px，这有些太多了。他们把宽度缩小为三分之二，所以`device-width`会返回给你一个320px的宽度，就像在iPhone上一样。

如果，像传闻那样，新的iPhone将会炫耀一个更大的像素数量（并不意味着一个更大的屏幕），我将不会感到惊讶如果苹果借鉴了这个行为。也许最终`device-width`就意味着320px。
...
* 其他媒体查询：dpi，orientation，aspect-ratio。尤其是dpi，那是一个灾难地区，不仅仅是因为所有浏览器都返回96dpi，有时候返回false，但是也因为我完全不确定对于web开发者来说哪个值是他们最感兴趣的。
