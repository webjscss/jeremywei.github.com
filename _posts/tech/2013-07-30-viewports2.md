...
这篇文章我们来聊聊关于移动浏览器的内容。如果你对移动开发完全是一个新手的话，我建议你先读一下[第一篇](http://weizhifeng.net/viewports.html)关于桌面浏览器的文章，在介绍下面的内容前先熟悉下环境。
...
平板设备中的像素中间层会在桌面环境和移动环境的缺口之间架起一段桥梁，比如像iPad或者传说中HP基于webOS研发的设备，但是这并没有改变根本问题。站点必须也能在移动设备上工作，所以我们不得不让它们能在小尺寸的屏幕上正常显示。
...
所以`<html>`元素在初始情况下用的是layout viewport的宽度，并且你的CSS也是在屏幕明显要比电话屏幕宽的假设基础上进行解释的。这使得你站点布局的行为与其在桌面浏览器上的一样。
